...
1. Then, check you are on the host where the VM is currently running using the host UUID:
...
1. Ensure `socat` is installed and execute this command with the DOM ID got earlier and a free TCP port:
...
1. Fine, now open a new shell, and on your local machine create an SSH tunnel with a free TCP port:

```
ssh -L <LOCAL_PORT>:localhost:<REMOTE_PORT> root@<HOST_IP>
```

1. Finally, start the client, for example `vncviewer`:
